...
router.get('/user', function (req, res, next) {
	var page_index = req.query.index || 1;
	sysUserService.getUsers(page_index, "", function (err, list, pagination) {
		if (err) {
			return next(err);
		}
		res.render('user/index', { username: req.user.username, title: "系统用户列表", list: list, pagination: pagination });
	});
});
router.get('/user/form', function (req, res, next) {
	var id = req.query.id || 0,
		title = id == 0 ? "添加系统用户" : "修改系统用户";
	sysUserService.getUserById(id, function (err, data) {
		if (err) {
			return next(err);
		}
		res.render('user/form', { username: req.user.username, title: title, user: data });
	});
});
router.post('/user/save', function (req, res, next) {
	var id = req.body.id || 0,
		name = req.body.name,
		pwd = req.body.pwd;
	sysUserService.saveUser(id, name, pwd, function (err, data) {
		if (err) {
			return next(err);
		}
		res.redirect('/user');
...
router.get('/type', function (req, res, next) {
	var page_index = req.query.index || 1;
	orangeTypeService.getTypes(page_index, "", function (err, list, pagination) {
		if (err) {
			return next(err);
		}
		res.render('type/index', { username: req.user.username, title: "类型列表", list: list, pagination: pagination });
	});
});
router.get('/type/form', function (req, res, next) {
	var id = req.query.id || 0,
		title = id == 0 ? "添加类型" : "修改类型";
	orangeTypeService.getTypeById(id, function (err, data) {
		if (err) {
			return next(err);
		}
		res.render('type/form', { username: req.user.username, title: title, type: data });
	});
});
router.post('/type/save', function (req, res, next) {
	var id = req.body.id || 0,
		name = req.body.name;
	orangeTypeService.saveType(id, name, function (err, data) {
		if (err) {
			return next(err);
		}
		res.redirect('/type');
...
router.get('/content', function (req, res, next) {
	var page_index = req.query.index || 1;
	orangeContentService.getContents(page_index, "", function (err, list, pagination) {
		if (err) {
			return next(err);
		}
		res.render('content/index', { username: req.user.username, title: "内容列表", list: list, pagination: pagination });
	});
});
router.get('/content/form', function (req, res, next) {
	var id = req.query.id || 0,
		title = id == 0 ? "添加内容" : "修改内容";
	orangeContentService.getContentById(id, function (err, data) {
		if (err) {
			return next(err);
		}
		res.render('content/form', { username: req.user.username, title: title, content: data });
	});
});
router.post('/content/save', function (req, res, next) {
	var id = req.body.id || 0,
		name = req.body.name;
	orangeContentService.saveContent(id, name, function (err, data) {
		if (err) {
			return next(err);
		}
